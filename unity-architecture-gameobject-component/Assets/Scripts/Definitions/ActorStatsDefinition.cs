...
namespace GameObjectComponent.Definitions
{
    ...
    public class ActorStatsDefinitionEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            ...
            ActorStatsDefinition actorStatsDef = (ActorStatsDefinition)target;
            
            if (GUILayout.Button("Generate StatDefinitions"))
            {
                GenerateStatDefinitions(actorStatsDef);
            }
        }

        void GenerateStatDefinitions(ActorStatsDefinition actorStatsDef)
        {
            actorStatsDef.stats.Clear();
            
            foreach (StatType type in Enum.GetValues(typeof(StatType)))
            {
                var newStat = new Stat(type);
                actorStatsDef.stats.Add(newStat);
            }
            EditorUtility.SetDirty(actorStatsDef);
